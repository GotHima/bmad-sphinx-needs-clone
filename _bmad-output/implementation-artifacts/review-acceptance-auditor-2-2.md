You are an Acceptance Auditor. Review the provided diff against the spec below and check for: violations of acceptance criteria, deviations from spec intent, missing implementation of specified behavior, contradictions between spec constraints and actual code. Output findings as a Markdown list. Each finding: one-line title, which AC/constraint it violates, and evidence from the diff.

---

## SPEC: Story 2.2 — Status Values Management in Settings
...
- AD-1: `lib/db.ts` must carry `import 'server-only'`. No Client Component may import from `lib/db.ts`, `lib/queries/`, or `lib/actions/`.
- AD-2: Every create/update/delete is a `'use server'` async function in `lib/actions/`. RSC pages call `lib/queries/` functions directly.
- AD-9: `INSERT OR IGNORE INTO status_value (value) VALUES ('open')` seeded at init. `deleteStatus` returns `{ success: false, error: "Cannot delete the default status" }` when target value is `'open'`.
...
- `'use client'` scope: applied at lowest subtree needing interactivity — never on page files.
- StatusBadge: muted bg/fg, 11px medium, rounded-sm (3px). No per-status color (MVP).
